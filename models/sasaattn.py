...
# four pooling layers
cfg_sasaattnnet = {
    'SASA_SM': ['C64', 'M', 128, 'M', 256, 'M', 512, 512, 'M'],
    'SASA_MD': ['C64', 'M', 128, 128, 'M', 256, 256, 'M', 512, 512, 'M'],
    'SASA_LG': ['C64', 64, 'M', 128, 128, 128, 'M', 256, 256, 256, 'M', 512, 512, 512, 'M'],
    'SASA_TI': ['C64', 64, 'M', 128, 128, 128, 'M', 256, 256, 256, 256, 'M', 512, 512, 512, 512, 'M'],
}


class SASAATTN(nn.Module):
    def __init__(self, sevgg_name):
        super(SASAATTN, self).__init__()
        ###############################
        assert sevgg_name in cfg_sasaattnnet
        ###############################
        self.features = self._make_layers(cfg_sasaattnnet[sevgg_name])
    ...
    def _make_layers(self, cfg):
        ...
        for x in cfg:
            if x == 'M':
                ...
                conv_channel = int(x[1:])
                layers += [
                    nn.Conv2d(channels, conv_channel,
                              kernel_size=3, padding=1),
                    nn.BatchNorm2d(conv_channel),
                    nn.ReLU(inplace=True),
                ]
                channels = conv_channel
            else:
                layers += [
                    SASA_Layer(channels, num_heads=8,
                               kernel_size=3, image_size=20),
                    nn.ReLU(inplace=True)
                ]
                channels = x
...
"""
Reference: https://github.com/MartinGer/Stand-Alone-Self-Attention-in-Vision-Models/blob/master/SASA_Layer.py
"""
...
class SASA_Layer(nn.Module):
    def __init__(self, in_channels, kernel_size=7, num_heads=8, image_size=224, inference=False):
        ...
        self.rel_encoding_h = nn.Parameter(torch.randn(
            self.dk // 2, self.kernel_size, 1), requires_grad=True)
        self.rel_encoding_w = nn.Parameter(torch.randn(
            self.dk // 2, 1, self.kernel_size), requires_grad=True)
    ...
    def forward(self, x):
        ...
        qk = qk.reshape(batch_size, self.num_heads, height,
        ...
        qk = qk.reshape(batch_size, self.num_heads, height,
                        width, 1, self.kernel_size*self.kernel_size)
